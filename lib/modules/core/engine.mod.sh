...
if command -v import.meta.loaded >/dev/null 2>&1; then
    import.meta.loaded "engine" "${BASH__PATH:-/opt/bash-lib}/modules/core/engine.mod.sh" "1.0.0" 2>/dev/null || true
fi
...
function engine.modules() {
    # List all module names (from *.mod.sh files in modules/)
    find "${BASH__PATH}/modules" -name "*.mod.sh" 2>/dev/null | while read -r modfile; do
...
function engine.modules.byCategory() {
    ...
    find ${BASH__PATH}/modules -type d -mindepth 1 -maxdepth 1 2>/dev/null | sort | while read -r category_dir; do
